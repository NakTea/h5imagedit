    ...
    ImageEditor.prototype = {
      ...
      _placeImage: function(image) {
        ...
        var $img = $(img),
            ...
            ratio = Math.max(containerWidth / naturalWidth, containerHeight / naturalHeight),
		...
		//alert(newWidth)
		//alert(newHeight)
		//alert(containerHeight - newHeight)
		//alert(containerWidth - newWidth)
      ...
      _onPan: function(e) {
		...
		if(image.height>=image.width){
			if(image.transform.translate.x>image.width*(image.transform.scale-1)/2){
				image.transform.translate.x = image.width*(image.transform.scale-1)/2;
			}else if(image.transform.translate.x<image.width*(1-image.transform.scale)/2){
				image.transform.translate.x = image.width*(1-image.transform.scale)/2;
			}
			var imgtop = (image.height - image.width/image.transform.scale)*(image.transform.scale)/2;
			if(image.transform.translate.y> imgtop ){
				image.transform.translate.y = imgtop;
			}else if(image.transform.translate.y< -imgtop ){
				image.transform.translate.y = -imgtop;
			}
		}else{
			if(image.transform.translate.y>image.height*(image.transform.scale-1)/2){
				image.transform.translate.y = image.height*(image.transform.scale-1)/2;
			}else if(image.transform.translate.y<image.height*(1-image.transform.scale)/2){
				image.transform.translate.y = image.height*(1-image.transform.scale)/2;
			}
			var imgtop = (image.width - image.height/image.transform.scale)*(image.transform.scale)/2;
			if(image.transform.translate.x> imgtop ){
				image.transform.translate.x = imgtop;
			}else if(image.transform.translate.x< -imgtop ){
				image.transform.translate.x = -imgtop;
			}
		}
		
		var deltax = image.transform.translate.x - this.startPan.x;
		var deltay = image.transform.translate.y - this.startPan.y;
		
		image.centerPoint = {
          x: this.startCenterPoint.x + deltax,
          y: this.startCenterPoint.y + deltay
        };//重新计算偏移量
      ...
      _drawImage: function(image, ctx, ratio) {
        var transform = image.transform;
        ...
        ctx.drawImage(image.img,
            ratio*(image.centerPoint.x - image.width * transform.scale / 2) / transform.scale,
            ratio*(image.centerPoint.y - image.height * transform.scale / 2) / transform.scale,
            image.width* ratio,
            image.height* ratio
        );
      ...
      selectImage: function(index) {
        ...
        this.images.forEach(function(image, i) {
          if(index == i)
            image.$mask.css('background', 'rgba(0,0,0,0.1)')
          else
      ...
      mergeImage: function() {
        ...
        cvs.width = this.options.width*ratio;
        cvs.height = this.options.height*ratio;
		ctx.fillStyle='#FFFFFF';
		ctx.fillRect(0,0,this.options.width*ratio,this.options.height*ratio);

        this.images.forEach(function(image) {
          that._drawImage(image, ctx, ratio);
        });
